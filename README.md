# image-processing
## This project contains all the jupyter notebook necessay to understand image processing (with Python).
   
French site : https://datacorner.fr/  
[Partie 1: L’image numérique](https://datacorner.fr/image-processing-1/)   
[Partie 2: Les histogrammes](https://datacorner.fr/image-processing-2/)     
[Partie 3: Seuillage d’image](https://datacorner.fr/image-processing-3/)      
[Partie 4: Transformations de base](https://datacorner.fr/image-processing-4/)      
[Partie 5: Transformations morphologiques](https://datacorner.fr/image-processing-5/)      
Partie 6: Filtres & Convolution  
Partie 7: Les réseaux de neurones à convolution (CNN)  

English site : http://aishelf.org/    
[Part 1: The digital](representation http://aishelf.org/image-processing-1/)  
[Part 2: The histogram](http://aishelf.org/image-processing-2/)    
[Part 3: Thresholding](http://aishelf.org/image-processing-3/)    
[Part 4: Transformations](http://aishelf.org/image-processing-4/)    
[Part 5: Morphologic](transformations http://aishelf.org/image-processing-5/)    
Part 5: Filters & convolution
Part 6: CNN